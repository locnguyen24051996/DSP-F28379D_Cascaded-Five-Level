...
void Initialize_PWM10(void)
{
    ...
    EPwm10Regs.TBPHS.bit.TBPHS = 0x0000;
...
*/
    ...
    EPwm12Regs.ETSEL.bit.SOCASEL = 1;
    EPwm12Regs.ETPS.bit.SOCAPRD = 1;
}
void GPIO_select(void)
{
    EALLOW;
...
/*--------------------PHA B-------------------------------------------*/
    if ((vb[index] >= 0) && (vb[index]<1))
         {
             EPWM5_CMPA =0;
             EPWM6_CMPA = 0;
             EPWM7_CMPA = 0;
             EPWM8_CMPA = TIMER_PERIOD* (vb[index]);
         }
     else if ((vb[index] >= 1) && (vb[index]<2))
     {
         EPWM5_CMPA = 0;
         EPWM6_CMPA = 0;
         EPWM7_CMPA = TIMER_PERIOD*(vb[index]-1);
         EPWM8_CMPA = TIMER_PERIOD;
     }

     else if ((vb[index] >= 2) && (vb[index]<3))
      {
          EPWM5_CMPA = 0;
     ...
     else if((vb[index] >=3) && (vb[index]<4))
          {
              EPWM5_CMPA = TIMER_PERIOD*(vb[index]-3) ;
              EPWM6_CMPA =TIMER_PERIOD;
              EPWM7_CMPA = TIMER_PERIOD;
              EPWM8_CMPA = TIMER_PERIOD;
          }
/*----------------------------PHA C--------------------------*/
    if ((vc[index] >= 0) && (vc[index]<1))
         {
             ...
             EPWM10_CMPA = 0;
             EPWM11_CMPA = 0;
             EPWM12_CMPA = TIMER_PERIOD* (vc[index]);
         }
     else if ((vc[index] >= 1) && (vc[index]<2))
     {
         EPWM9_CMPA = 0;
         EPWM10_CMPA = 0;
         EPWM11_CMPA = TIMER_PERIOD*(vc[index]-1);
         EPWM12_CMPA = TIMER_PERIOD;
     }

     else if ((vc[index] >= 2) && (vc[index]<3))
      {
          EPWM9_CMPA = 0;
     ...
     else if((vc[index] >=3) && (vc[index]<4))
          {
              EPWM9_CMPA = TIMER_PERIOD*(vc[index]-3) ;
              EPWM10_CMPA =TIMER_PERIOD;
              EPWM11_CMPA = TIMER_PERIOD;
              EPWM12_CMPA = TIMER_PERIOD;
          }
