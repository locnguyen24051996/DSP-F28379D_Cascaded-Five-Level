...
void Initialize_PWM2(void)
{
    ...
    EPwm2Regs.TBPHS.bit.TBPHS = TIMER_PERIOD;
...
void Initialize_PWM6(void)
{
    ...
    EPwm6Regs.TBPHS.bit.TBPHS = TIMER_PERIOD;
...
void Initialize_PWM10(void)
{
    ...
    EPwm10Regs.TBPHS.bit.TBPHS = TIMER_PERIOD;
    ...
    EPwm10Regs.TBCTL.bit.PHSDIR = 0x0;
...
void Initialize_PWM11(void)
{
    ...
    EPwm11Regs.TBPHS.bit.TBPHS = 0x0000;
...
void Initialize_PWM12(void)
{
    ...
    EPwm12Regs.TBPHS.bit.TBPHS = 0x0000;
...
__interrupt void epwm1_isr(void)
{
  ...
  /*  if(AdcaRegs.ADCINTFLG.bit.ADCINT1 == 1)
    ...
    ///////////////////////////////////////////
/*----------------PHA A-------------------------*/
    if ((va[index] >=0)&&( va[index] <2))
      {
          EPWM1_CMPA = TIMER_PERIOD * ((va[index])/2);
          EPWM2_CMPA = TIMER_PERIOD * ((va[index])/2);
          EPWM3_CMPA = 0;
          EPWM4_CMPA = 0;
      }
      else if ((va[index] >=2)&&( va[index] <4))
      {
          EPWM1_CMPA = TIMER_PERIOD * ((va[index]-2)/2);
          EPWM2_CMPA = TIMER_PERIOD * ((va[index]-2)/2);
          EPWM3_CMPA = TIMER_PERIOD;
          EPWM4_CMPA = TIMER_PERIOD;
      }
/*------------------------PHA B----------------*/
    if ((vb[index] >=0)&&( vb[index] <2))
         {
             EPWM5_CMPA = TIMER_PERIOD * ((vb[index])/2);
             EPWM6_CMPA = TIMER_PERIOD * ((vb[index])/2);
             EPWM7_CMPA = 0;
             EPWM8_CMPA = 0;
         }
         else if ((vb[index] >=2)&&( vb[index] <4))
         {
             EPWM5_CMPA = TIMER_PERIOD * ((vb[index]-2)/2);
             EPWM6_CMPA = TIMER_PERIOD * ((vb[index]-2)/2);
             EPWM7_CMPA = TIMER_PERIOD;
             EPWM8_CMPA = TIMER_PERIOD;
         }
/*---------------------PHA C---------------------*/
    if ((vc[index] >=0)&&( vc[index] <2))
         {
             EPWM9_CMPA = TIMER_PERIOD * ((vc[index])/2);
             EPWM10_CMPA = TIMER_PERIOD * ((vc[index])/2);
             EPWM11_CMPA = 0;
             EPWM12_CMPA = 0;
         }
         else if ((vc[index] >=2)&&( vc[index] <4))
         {
             EPWM9_CMPA = TIMER_PERIOD * ((vc[index]-2)/2);
             EPWM10_CMPA = TIMER_PERIOD * ((vc[index]-2)/2);
             EPWM11_CMPA = TIMER_PERIOD;
             EPWM12_CMPA = TIMER_PERIOD;
         }
